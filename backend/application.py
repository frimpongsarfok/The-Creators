...
CORS(application)  # Add closing parenthesis here

application.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:////Users/kantanka/Desktop/The-Operators-1/backend/instance/mydatabase.db'
...
@application.route('/update_all_rooms', methods=['GET'])
...
@application.route('/update_user_passoword', methods=['POST'])
def create_user():
    """
...
@application.route('/room', methods=['POST'])
def create_room():
    data = request.json
...
@application.route('/available_rooms', methods=['GET'])
def get_available_rooms():
    rooms = Room.query.all()
    rooms_list = []
    for room in rooms:
        rooms_list.applicationend({
            'id': room.id,
            'price': room.price,
            'type': room.type,
            'available_rooms': room.available_rooms
...
@application.route('/receipt', methods=['GET'])
def get_receipt():
...
# Run the Flask applicationlication
